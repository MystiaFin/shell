...
Item {
    ...
    Process {
        ...
        stdout: SplitParser { onRead: data => console.log("[Connect Out]: " + data) }
        stderr: SplitParser { onRead: data => console.log("[Connect Err]: " + data) }
        onExited: {
    ...
    Process {
        ...
        stdout: SplitParser { onRead: data => service.isEnabled = data.trim() === "enabled" }
    }
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                const lines = data.split('\n')
                for (const line of lines) {
                    if (line.trim() !== "") service._knownList.push(line.trim())
                }
            }
        }
    ...
    Process {
        ...
        onExited: {
            ...
            for (const newItem of uniqueResults) {
                ...
                if (foundIndex !== -1) networks.set(foundIndex, newItem);
                else networks.append(newItem);
            }
            
            for (let i = networks.count - 1; i >= 0; i--) {
                const oldSSID = networks.get(i).ssid;
                if (!seenSSIDs.has(oldSSID)) networks.remove(i);
            }
        }
    }
    ...
    Timer { id: delayedUpdate; interval: 2000; onTriggered: service.update() }
    
    Component.onCompleted: update()
    
    function update() {
        if (typing) return;
        if (connectionProcess.running) return; // Busy
